...
**My Agent** is an LLM-based agent for general purpose software engineering tasks, adapted from [Trae Agent](https://github.com/trae-ai/trae-agent). It provides a powerful CLI interface that can understand natural language instructions and execute complex software engineering workflows using various tools and LLM providers.

**Recent Updates:** This version includes significant bug fixes and improvements:
- ✅ **Fixed ToolCall Attribute Errors**: Resolved multiple `'ToolCall' object has no attribute 'function'` errors across the codebase
- ✅ **Enhanced Configuration**: Improved API key loading from environment variables with proper fallback handling
- ✅ **Updated Dependencies**: Fixed compatibility issues with modern Python packages
- ✅ **Working Demo**: Successfully creates and executes Python scripts (see `hello_world.py` example)

**Difference with Other CLI Agents:** My Agent offers a transparent, modular architecture that researchers and developers can easily modify, extend, and analyze. The open-source nature allows for deep customization of agent behaviors, tool implementations, and workflow patterns, making it an ideal platform for **studying AI agent architectures, conducting ablation studies, and developing novel agent capabilities**. This ***research-friendly design*** enables the academic and open-source communities to contribute to and build upon the foundational agent framework, fostering innovation in the rapidly evolving field of AI agents.

**Project Status:** My Agent is currently functional with core features working. The codebase has been debugged and tested with successful script generation. We continue to work on supporting more LLM providers, enriching the command-line interface, developing more tools and adding MCP support. Building a robust unit testing framework remains a priority.
...
There are three ways to configure API keys for My Agent:
...
DOUBAO_API_BASE_URL=your-model-provider-base-url
OPENROUTER_API_KEY=your-openrouter-api-key-here
```

#### Option 2: Using Configuration File

Configure My Agent using the config file (see Configuration section below).
...
# Run with Doubao
my-agent-cli run "Create a hello world Python script" --provider doubao --model doubao-seed-1.6
```
...
The main entry point is the `my-agent-cli` command with several subcommands:

#### `my-agent-cli run` - Execute a Task

```bash
# Basic task execution
my-agent-cli run "Create a Python script that calculates fibonacci numbers"

# With specific provider and model
my-agent-cli run "Fix the bug in main.py" --provider anthropic --model claude-sonnet-4-20250514

# Using OpenRouter with any supported model
my-agent-cli run "Optimize this code" --provider openrouter --model "openai/gpt-4o"
my-agent-cli run "Add documentation" --provider openrouter --model "anthropic/claude-3-5-sonnet"

# With custom working directory
my-agent-cli run "Add unit tests for the utils module" --working-dir /path/to/project

# Save trajectory for debugging
my-agent-cli run "Refactor the database module" --trajectory-file debug_session.json

# Force to generate patches
my-agent-cli run "Update the API endpoints" --must-patch
```

#### `my-agent-cli interactive` - Interactive Mode

```bash
# Start interactive session
my-agent-cli interactive

# With custom configuration
my-agent-cli interactive --provider openai --model gpt-4o --max-steps 30
```
